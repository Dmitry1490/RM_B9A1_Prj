...
///=======================================================================================================/
/// ??? ????????????? TA1-USB-01-M (???<->USB)
/// ????????? ????? "USB_TA_DRV.dll" ? "USB_TA_VC2.dll" ?? ???????? "i386"
/// ??? ????? ?????? ???? ???????? ? debug ??? release
/// "f:/WORK/SPP/PSI_BALK2M/PSI_1/build-PSI_1-Desktop_Qt_5_15_0_MinGW_32_bit-Debug/debug/USB_TA_DRV.dll"
///=======================================================================================================/
...
static HANDLE _ahVTMK4Event[MAX_TMK_NUMBER+1];
static HANDLE _hVTMK4VxD[MAX_TMK_NUMBER+1];
static int _VTMK4tmkMaxNum = -1;
static int _VTMK4mrtMaxNum = -1;
static int _VTMK4tmkCurNum = 0;
static char _VTMK4usbFlag[MAX_TMK_NUMBER+1];
static int _VTMK4tmkDrvnum[MAX_TMK_NUMBER+1];

static HINSTANCE hUSBDrvDll = nullptr;
...
DWORD TmkOpen()
{
  ...
  for (iTMK = 0; iTMK <= MAX_TMK_NUMBER; ++iTMK)
  {
    ...
    {
      if (!DeviceIoControl(_hVTMK4VxD[iTMK], (DWORD)IOCTL_TMK_GetVersion, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr) ||
    ...
    _hVTMK4VxD[iTMK] = nullptr;
    _ahVTMK4Event[iTMK] = nullptr;
  }
  if(badver)
  ...
  if(!hUSBDrvDll)
  {
    //hUSBDrvDll = LoadLibrary(TEXT("f:/WORK/SPP/PSI_BALK2M/PSI_1/build-PSI_1-Desktop_Qt_5_15_0_MinGW_32_bit-Debug/debug/USB_TA_DRV.dll"));
    hUSBDrvDll = LoadLibrary(TEXT("USB_TA_DRV.dll"));
  }

  if(hUSBDrvDll != nullptr)
  {
    ...
    if(loaded)
    {
      ...
      for(iTMK = 0; iTMK <= MAX_USBTA_NUM; ++iTMK)
      {
        ...
        {
          DeviceIoControl(_hVTMK4VxD[nMRT+1+nTMKb], (DWORD)IOCTL_EZUSB_GET_DRIVER_VERSION, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 6, &_cbVTMK4Ret, nullptr);
          ...
          _hVTMK4VxD[nMRT+1+nTMKb] = nullptr;
          nTMKb++;
        }
        else
          _hVTMK4VxD[nMRT+1+nTMKb] = nullptr;
...
int tmkconfig(int tmkNumber)
{
  ...
  if (_VTMK4tmkMaxNum == -1)
    return static_cast<int>(TMK_BAD_NUMBER);

  if (_hVTMK4VxD[tmkNumber] != nullptr)
    return static_cast<int>(TMK_BAD_NUMBER);
  ...
  if (_VTMK4usbFlag[tmkNumber])
  {
    ...
    {
      _hVTMK4VxD[tmkNumber] = nullptr;
      return static_cast<int>(TMK_BAD_NUMBER);
    }

    if(DeviceIoControl(_hVTMK4VxD[tmkNumber], (DWORD)IOCTL_EZUSB_SET_CURPROC, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr))
    {
  ...
  DeviceIoControl(_hVTMK4VxD[tmkNumber], (DWORD)IOCTL_TMK_tmkconfig, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
  ...
  _VTMK4tmkCurNum = tmkNumber;
  return static_cast<int>(_awVTMK4OutBuf[0]);
}
...
int tmkdone(int tmkNumber)
{
  ...
  if (tmkNumber == ALL_TMKS)
  {
    for (iTMK = 0; iTMK <= _VTMK4tmkMaxNum; ++iTMK)
    {
      ...
      {
        if (_hVTMK4VxD[iTMK] == nullptr)
          continue;
        _awVTMK4InBuf[0] = (WORD)iTMK;
        DeviceIoControl(_hVTMK4VxD[iTMK], (DWORD)IOCTL_TMK_tmkdone, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  ...
  else if (tmkNumber >= 0 && tmkNumber <= _VTMK4tmkMaxNum && _hVTMK4VxD[tmkNumber] != NULL)
  {
    if(_VTMK4usbFlag[tmkNumber])
    {
      _ahVTMK4Event[tmkNumber] = nullptr;
      CloseHandle (_hVTMK4VxD[tmkNumber]);
      _hVTMK4VxD[tmkNumber] = nullptr;
    ...
    {
      _awVTMK4InBuf[0] = (WORD)tmkNumber;
      DeviceIoControl(_hVTMK4VxD[tmkNumber], (DWORD)IOCTL_TMK_tmkdone, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
...
int tmkselect(int tmkNumber)
{
  ...
  if ((_hVTMK4VxD[tmkNumber] != INVALID_HANDLE_VALUE) && (_hVTMK4VxD[tmkNumber] != nullptr))
  {
    if (_VTMK4usbFlag[tmkNumber])
    {
      _VTMK4tmkCurNum = tmkNumber;
    ...
    DeviceIoControl(_hVTMK4VxD[tmkNumber], (DWORD)IOCTL_TMK_tmkselect, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
...
TMK_DATA tmkgetmode()
{
  ...
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgetmode, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
void tmksetcwbits(TMK_DATA tmkSetControl)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmksetcwbits, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
void tmkclrcwbits(TMK_DATA tmkClrControl)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkclrcwbits, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
TMK_DATA tmkgetcwbits()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgetcwbits, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
*/
  ...
  _awVTMK4InBuf[3] = HIWORD(fEventSet);
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkdefevent, _awVTMK4InBuf, 8, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    tmkdefevent_usb(hEvent);
#endif
}
...
void tmkgetevd(TTmkEventData *pEvD)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgetevd, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 12, &_cbVTMK4Ret, nullptr);
...
void tmkgetinfo(TTmkConfigData *pConfD)
{
  ...
  *(PDWORD64)(_awVTMK4InBuf) = (DWORD64)pConfD;
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgetinfo, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  }
#endif
  else
  {
...
int bcreset()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return(DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcreset, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
    return bcreset_usb();
}
...
void bc_def_tldw(TMK_DATA wTLDW)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bc_def_tldw, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
}
...
void bc_enable_di()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bc_enable_di, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
}
...
int bcdefirqmode(TMK_DATA bcIrqMode)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcdefirqmode, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA bcgetirqmode()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetirqmode, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
TMK_DATA bcgetmaxbase()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetmaxbase, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
int bcdefbase(TMK_DATA bcBasePC)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcdefbase, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA bcgetbase()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetbase, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
void bcputw(TMK_DATA bcAddr, TMK_DATA bcData)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcputw, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
TMK_DATA bcgetw(TMK_DATA bcAddr)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetw, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
DWORD bcgetansw(TMK_DATA bcCtrlCode)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetansw, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
...
//  _awVTMK4InBuf[2] = 0;
  ...
  *(PDWORD64)(_awVTMK4InBuf+4) = (DWORD64)pcBuffer;
  ...
  _awVTMK4InBuf[3] = cwLength;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcputblk, _awVTMK4InBuf, 8, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    bcputblk_usb(bcAddr, pcBuffer, cwLength);
#endif
}
...
//  _awVTMK4InBuf[2] = 0;
  ...
  *(PDWORD64)(_awVTMK4InBuf+4) = (DWORD64)pcBuffer;
  ...
  _awVTMK4InBuf[3] = cwLength;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetblk, _awVTMK4InBuf, 8, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    bcgetblk_usb(bcAddr, pcBuffer, cwLength);
#endif
}
...
int bcdefbus(TMK_DATA bcBus)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcdefbus, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA bcgetbus()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetbus, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
    return bcgetbus_usb();
...
int bcstart(TMK_DATA bcBase, TMK_DATA bcCtrlCode)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcstart, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
int bcstartx(TMK_DATA bcBase, TMK_DATA bcCtrlCode)
{
  ...
  _awVTMK4InBuf[1] = bcCtrlCode;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcstartx, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
int bcdeflink(TMK_DATA bcBase, TMK_DATA bcCtrlCode)
{
  ...
  _awVTMK4InBuf[1] = bcCtrlCode;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcdeflink, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
DWORD bcgetlink()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetlink, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
    return (((DWORD*)(_awVTMK4OutBuf))[0]);
  }
  else
...
TMK_DATA bcstop()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcstop, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
    return bcstop_usb();
...
DWORD bcgetstate()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetstate, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
    return (((DWORD*)(_awVTMK4OutBuf))[0]);
  }
  else
...
int rtreset()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtreset, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
    return rtreset_usb();
}
...
int rtdefirqmode(TMK_DATA rtIrqMode)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtdefirqmode, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA rtgetirqmode()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetirqmode, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
int rtdefmode(TMK_DATA rtMode)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtdefmode, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA rtgetmode()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetmode, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
TMK_DATA rtgetmaxpage()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetmaxpage, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
int rtdefpage(TMK_DATA rtPage)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtdefpage, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA rtgetpage()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetpage, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
int rtdefpagepc(TMK_DATA rtPagePC)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtdefpagepc, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
int rtdefpagebus(TMK_DATA rtPageBus)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtdefpagebus, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA rtgetpagepc()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetpagepc, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
TMK_DATA rtgetpagebus()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetpagebus, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
int rtdefaddress(TMK_DATA rtAddress)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtdefaddress, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA rtgetaddress()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetaddress, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
void rtdefsubaddr(TMK_DATA rtDir, TMK_DATA rtSubAddr)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtdefsubaddr, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
TMK_DATA rtgetsubaddr()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetsubaddr, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
void rtputw(TMK_DATA rtAddr, TMK_DATA rtData)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtputw, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
TMK_DATA rtgetw(TMK_DATA rtAddr)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetw, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
//  _awVTMK4InBuf[2] = 0;
  ...
  *(PDWORD64)(_awVTMK4InBuf+4) = (DWORD64)pcBuffer;
  ...
  _awVTMK4InBuf[3] = cwLength;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtputblk, _awVTMK4InBuf, 8, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    rtputblk_usb(rtAddr, pcBuffer, cwLength);
#endif
}
...
//  _awVTMK4InBuf[2] = 0;
  ...
  *(PDWORD64)(_awVTMK4InBuf+4) = (DWORD64)pcBuffer;
  ...
  _awVTMK4InBuf[3] = cwLength;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetblk, _awVTMK4InBuf, 8, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    rtgetblk_usb(rtAddr, pcBuffer, cwLength);
#endif
}
...
void rtsetanswbits(TMK_DATA rtSetControl)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtsetanswbits, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
TMK_DATA rtgetanswbits()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetanswbits, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
//  _awVTMK4InBuf[1] = 0;
  ...
  *(PDWORD64)(_awVTMK4InBuf+8) = (DWORD64)pcBuffer;
  ...
  _awVTMK4InBuf[4] = rtFlagMax;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetflags, _awVTMK4InBuf, 10, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    rtgetflags_usb(pcBuffer, rtDir, rtFlagMin, rtFlagMax);
#endif
}
...
//  _awVTMK4InBuf[1] = 0;
  ...
  *(PDWORD64)(_awVTMK4InBuf+8) = (DWORD64)pcBuffer;
  ...
  _awVTMK4InBuf[4] = rtFlagMax;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtputflags, _awVTMK4InBuf, 10, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    rtputflags_usb(pcBuffer, rtDir, rtFlagMin, rtFlagMax);
#endif
}
...
void rtsetflag()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtsetflag, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    rtsetflag_usb();
}
...
void rtclrflag()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtclrflag, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    rtclrflag_usb();
}
...
TMK_DATA rtgetstate()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetstate, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
TMK_DATA rtbusy()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtbusy, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
    return rtbusy_usb();
...
void rtlock(TMK_DATA rtDir, TMK_DATA rtSubAddr)
{
  ...
  _awVTMK4InBuf[1] = rtSubAddr;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtlock, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
void rtunlock()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtunlock, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
    rtunlock_usb();
}
...
TMK_DATA rtgetcmddata(TMK_DATA rtBusCommand)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetcmddata, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
void rtputcmddata(TMK_DATA rtBusCommand, TMK_DATA rtData)
{
  ...
  _awVTMK4InBuf[1] = rtData;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtputcmddata, _awVTMK4InBuf, 4, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
int mtreset()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mtreset, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
    return mtreset_usb();
}
...
TMK_DATA mtgetsw()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mtgetsw, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
    return mtgetsw_usb();
...
TMK_DATA rtenable(TMK_DATA rtEnable)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtenable, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
DWORD mrtconfig(int mrtNumber)
{
  ...
  if (_hVTMK4VxD[mrtNumber] != nullptr)
    return 0;
  ...
  if (!_VTMK4usbFlag[mrtNumber])
  {
    ...
    {
      _hVTMK4VxD[mrtNumber] = nullptr;
      return 0;
    }

    _awVTMK4InBuf[0] = (WORD)mrtNumber;
    DeviceIoControl(_hVTMK4VxD[mrtNumber], (DWORD)IOCTL_TMK_mrtconfig, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
...
int mrtselected()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mrtselected, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return ((int)(_awVTMK4OutBuf[0]));
  }
  else
...
TMK_DATA mrtgetstate()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mrtgetstate, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
void mrtdefbrcsubaddr0()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mrtdefbrcsubaddr0, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr);
  else
...
int mrtreset()
{
  ...
  DWORD _cbVTMK4Ret;
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mrtreset, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
    return mrtreset_usb();
}
...
TMK_DATA tmktimer(TMK_DATA tmkTimerCtrl)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmktimer, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
DWORD tmkgettimer()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgettimer, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
    return (((DWORD*)(_awVTMK4OutBuf))[0]);
  }
  else
...
TMK_DATA tmkgettimerl()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgettimerl, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
DWORD bcgetmsgtime()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_bcgetmsgtime, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
    return (((DWORD*)(_awVTMK4OutBuf))[0]);
  }
  else
...
DWORD rtgetmsgtime()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_rtgetmsgtime, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
    return (((DWORD*)(_awVTMK4OutBuf))[0]);
  }
  else
...
TMK_DATA tmkgethwver()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgethwver, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
DWORD tmkgetevtime()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgetevtime, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
    return (((DWORD*)(_awVTMK4OutBuf))[0]);
  }
  else
...
TMK_DATA tmkswtimer(TMK_DATA tmkTimerCtrl)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkswtimer, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
DWORD tmkgetswtimer()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmkgetswtimer, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 4, &_cbVTMK4Ret, nullptr);
    return (((DWORD*)(_awVTMK4OutBuf))[0]);
  }
  else
...
TMK_DATA tmktimeout(TMK_DATA tmkTimeOut)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_tmktimeout, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
int mrtdefbrcpage(TMK_DATA mrtBrcPage)
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mrtdefbrcpage, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
...
TMK_DATA mrtgetbrcpage()
{
  ...
  if (!_VTMK4usbFlag[_VTMK4tmkCurNum])
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mrtgetbrcpage, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
...
int mbcinit(TMK_DATA mbcId)
{
  ...
  if(_VTMK4usbFlag[_VTMK4tmkCurNum] == 0)
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mbcinit, _awVTMK4InBuf, 2, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
    return TMK_BAD_NUMBER;
}
...
int mbcpreparex(TMK_DATA mbcId, TMK_DATA bcBase, TMK_DATA bcCtrlCode, TMK_DATA mbcDelay)
{
  ...
  if(_VTMK4usbFlag[_VTMK4tmkCurNum] == 0)
    return (DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mbcpreparex, _awVTMK4InBuf, 8, _awVTMK4OutBuf, 0, &_cbVTMK4Ret, nullptr))?0:GetLastError();
  else
    return TMK_BAD_NUMBER;
}
...
TMK_DATA mbcalloc()
{
  ...
  if(_VTMK4usbFlag[_VTMK4tmkCurNum] == 0)
  {
    DeviceIoControl(_hVTMK4VxD[_VTMK4tmkCurNum], (DWORD)IOCTL_TMK_mbcalloc, _awVTMK4InBuf, 0, _awVTMK4OutBuf, 2, &_cbVTMK4Ret, nullptr);
    return _awVTMK4OutBuf[0];
  }
  else
    return 0;
